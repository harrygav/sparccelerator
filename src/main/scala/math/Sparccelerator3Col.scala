...
object Sparccelerator3Col {

  @nativeLoader("sparccelerator1")
  object NativeMethod {
    @native def minmax3(buf1Off: Int, buf1Len: Int, buf1: ByteBuffer, buf2: ByteBuffer, buf3: ByteBuffer,
      resBuf: ByteBuffer): Void
  }
  ...
  def main(args: Array[String]) {
    ...
    val spark = SparkSession.builder.appName("Sparccelerator minmax 3 cols").getOrCreate()
    ...
    val minmaxs = tupleData.mapPartitions(tuples => {
      ...
      val bb2 = ByteBuffer.allocateDirect(8 * batchSize).order(ByteOrder.nativeOrder())
      val bb3 = ByteBuffer.allocateDirect(8 * batchSize).order(ByteOrder.nativeOrder())
      ...
      tuples.foreach(tuple => {
        ...
        if (cnt == batchSize) {
          cnt = 0
          NativeMethod.minmax3(0, batchSize, bb1, bb2, bb3, resBuffer)
